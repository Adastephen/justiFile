  ...
  const { data: cases, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("cases")
        .select(
          "id, title, category, jurisdiction, is_pro_bono, budget_ngn, description, created_at",
        )
        .is("lawyer_id", null)
        .eq("status", "open")
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="font-display text-4xl mt-2">
              Open matters awaiting counsel
            </h1>
            <p className="text-muted-foreground mt-2 text-sm">
              Claim a case to open a private channel with the client.
            </p>
          </div>
          <Link to="/dashboard" className="btn-ghost-gold">
          ...
          {!isLoading && (!cases || cases.length === 0) && (
            <div className="rounded-xl border border-border bg-panel/60 p-10 text-center">
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Check back shortly. Pro bono and paid matters surface here as
                clients submit them.
              </p>
            </div>
          )}
            ...
            >
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="flex gap-2">
                  {c.is_pro_bono ? (
                    <span className="text-xs px-2 py-1 rounded-full bg-naija/15 text-naija">
                      Pro bono
                    </span>
                  ) : (
                    <span className="text-xs px-2 py-1 rounded-full bg-gold/15 text-gold">
                      {c.budget_ngn
                        ? `₦${c.budget_ngn.toLocaleString()}`
                        : "Paid"}
                    </span>
                  )}
                </div>
